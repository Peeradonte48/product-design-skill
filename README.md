...
| **[page-to-figma](skills/page-to-figma/SKILL.md)** | running page → Figma | Transcribes a **running** product page into a 1:1 Figma frame. Extracts live-DOM computed styles as ground truth, delegates the build to the vendored figma-cli, then gates on a **numeric property read-back** — correcting until every value matches. page-to-figma drives a vendored, locally-running figma-cli (no Figma API token, no rate limits) and falls back to the official Figma plugin only when the CLI isn't connected. *(Requires the official Figma plugin as fallback.)* |
