...
Twelve skills across four groups. Most go **design/flow → code or doc**; `page-to-figma` is the one that runs the other way (running page → Figma wireflow).
...
| **[page-to-figma](skills/page-to-figma/SKILL.md)** | running page → Figma wireflow | Captures a running product page (or several) into Figma using Figma's **native** `generate_figma_design` capture, then arranges the frames and connects them with **magnetic, auto-rerouting arrows** into a **wireflow**. Flow sources: explicit transition list, FigJam/UCN, or a **bounded crawl** the user confirms. It **no longer reconstructs** pages node-by-node — fidelity is Figma's capture's job. Node ops run via **`use_figma`** (Figma MCP Plugin API); arrows are **magnetic FigJam connectors** cloned from a one-time human-pasted **donor** (static VECTOR arrows are the no-donor fallback). The vendored figma-cli is a fallback engine only. **Hard, fail-closed dependency** on the Figma MCP capture (+ Playwright for page/auth driving). *(Requires the official Figma plugin + Playwright.)* |
...
```
  ...
  Running page(s)  ──▶  page-to-figma  ──▶  captured frames + magnetic-connector wireflow in Figma
...
- For `page-to-figma` only: **hard, fail-closed dependency** on the **official Figma plugin** (for the native `generate_figma_design` capture — this is the primary engine, not a fallback) **and Playwright** (to drive page navigation and auth — prefer the Playwright MCP). The skill stops with setup instructions if either is missing. Node ops (container/reparent/arrange/connect) run via the Figma plugin's **`use_figma`** Plugin API (primary, the same bridge as capture); the vendored figma-cli is a **fallback** only (its local CDP bridge can fail outright). Arrows are **magnetic FigJam connectors** cloned from a one-time human-pasted **donor** connector (`createConnector` is unavailable in a design file, but a pasted connector survives and is clonable) — static VECTOR arrows are the no-donor fallback. Capture also needs an in-page code-execution permission (e.g. the Playwright MCP's `browser_run_code_unsafe`), which the user grants once. See [docs/adr/0008-page-to-figma-magnetic-connectors.md](docs/adr/0008-page-to-figma-magnetic-connectors.md) and [docs/adr/0007-page-to-figma-capture-wireflow.md](docs/adr/0007-page-to-figma-capture-wireflow.md) (supersedes ADR 0006, updates [ADR 0001](docs/adr/0001-page-to-figma-depends-on-official-figma-plugin.md)).
...
├── page-to-figma/
│   ├── SKILL.md                             # running page → Figma wireflow (capture + arrange + connect)
│   └── references/
│       └── wireflow-build.md                # capture/poll, container, arrange, magnetic connectors, auth, crawl
...
└── adr/
    ...
    ├── 0006-page-to-figma-vendored-figma-cli.md
    ├── 0007-page-to-figma-capture-wireflow.md          # capture+wireflow replaces reconstruction
    └── 0008-page-to-figma-magnetic-connectors.md       # magnetic connectors + use_figma primary engine
```
