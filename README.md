...
| **[page-to-figma](skills/page-to-figma/SKILL.md)** | running page → Figma wireflow | Captures a running product page (or several) into Figma using Figma's **native** `generate_figma_design` capture, then arranges the frames and connects them with labeled arrows into a **wireflow**. Flow sources: explicit transition list, FigJam/UCN, or a **bounded crawl** the user confirms. It **no longer reconstructs** pages node-by-node — fidelity is Figma's capture's job. The vendored figma-cli survives only as an `eval` helper for arrange + arrows. **Hard, fail-closed dependency** on the Figma MCP capture (+ Playwright for page/auth driving). *(Requires the official Figma plugin + Playwright.)* |
...
```
  ...
  Running page(s)  ──▶  page-to-figma  ──▶  captured frames + labeled-arrow wireflow in Figma
...
- For `page-to-figma` only: **hard, fail-closed dependency** on the **official Figma plugin** (for the native `generate_figma_design` capture — this is the primary engine, not a fallback) **and Playwright** (to drive page navigation and auth). The skill stops with setup instructions if either is missing. The vendored figma-cli is retained only as an `eval` helper for frame arrangement and arrow drawing. See [docs/adr/0007-page-to-figma-capture-wireflow.md](docs/adr/0007-page-to-figma-capture-wireflow.md) (supersedes ADR 0006, updates [ADR 0001](docs/adr/0001-page-to-figma-depends-on-official-figma-plugin.md)).
...
├── page-to-figma/
│   └── SKILL.md                             # running page → Figma wireflow (capture + arrange + arrows)
